		                            ...
		                            db.smtCompanies.findOne({_id: report.companyId}, function(errr, company){
			                            ...
			                            db.users.findOne({_id: report.userId}, function(err, user){
			                            	if (user && user.profile && user.profile.locationId) {
			                            		...
			                            		toArrayData(report, user, function(toData){
			                            			var to = _.uniq(toData);
			                            			if (report.reportCode === "APPOINTMENTS") {
					                                    endDayAppointments(report._id, "SYSTEM", function(ress){
					                                    	utils.saveNotification(notificationObj, from, to, function(re){
...
function toArrayData(report, user, callback){
	var to = [];
...
function endDayAppointments(reportId,createdBy, callback){
	db.smtReportDetails.findOne({_id: reportId}, function(err, report){
	    ...
	    updateUnAttendedAppointment(reportDate, report.userId, function(ress){
		   ...
		   // console.log(options);
	    	db.smtCustomerAppointments.find({
			    ...
			    db.smtCustomerAppointments.update({_id: {$in: appointmentIds}},{$set: {summarySubmitted:true}},function(err, doc){});
			    db.smtCustomerAppointmentsSummary.update({appointmentId: {$in: appointmentIds}}, {$set: {summarySubmitted: true}}, function(err, doc){});
			   	var options = {
			    	mrId: report.userId,
...
function updateUnAttendedAppointment(tripDate,soId, callback){
	if (tripDate) {
        ...
        db.users.findOne({_id:userId}, function(err, createdBy){
            ...
            }, {$set: {appointmentStatus: "CANCELLED"},$push:{statusAudit:statusAuditObj}}, function(err, doc){
            	if(err == null){
			        ...
			        },{$set: {meetingStatus: "CANCELLED"},$push:{statusAudit:statusAuditObj}}, function(err, doc){
			        	if(err == null){
			        		//console.log('update done');
